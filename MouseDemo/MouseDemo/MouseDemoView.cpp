...
void CMouseDemoView::OnPaint()
{
	...
	strTemp.Format(_T("안녕하세요, %d, %d 반가워요"), m_nX, m_nY);
	dc.TextOut(10, 10, strTemp);*/

	dc.TextOut(m_nX, m_nY, _T("내 컴터"));
...
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	if (m_bDrag == TRUE) // 지금 drag 중이니?
	{
		m_nX = point.x;
		m_nY = point.y;
		RedrawWindow(); // WM_POINT 발생
	}
