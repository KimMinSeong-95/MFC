...
int CCreateDemoView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_wndChild.Create(_T("BUTTON"), _T("김민성"),
		WS_CHILD | WS_VISIBLE | WS_BORDER,
		CRect(30, 30, 180, 180), /*GetDesktopWindow()*/this, 1234);
...
void CCreateDemoView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	// 1. 현재 버튼의 위치를 구한다
	CRect rectButton;
	this->m_wndChild.GetWindowRect(&rectButton); // 스크린 좌표계
	ScreenToClient(&rectButton);// 클라이언트 좌표계

	int x = rectButton.left;
	int y = rectButton.top;
	int w = rectButton.Width(); //width
	...
	// 3. 버튼을 계산된 위치로 이동한다.
	m_wndChild.SetWindowPos(NULL, x, y, w, h, SWP_NOZORDER | SWP_SHOWWINDOW);
...
void CCreateDemoView::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	CString strTemp;
	strTemp.Format(_T("%d, %c키 눌림"), nChar, nChar);
	//AfxMessageBox(strTemp);

	m_wndChild.SetWindowText(strTemp); // 버튼
	SetWindowText(strTemp); // 뷰
