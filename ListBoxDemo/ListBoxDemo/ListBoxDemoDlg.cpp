...
void CListBoxDemoDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_CBString(pDX, IDC_COMBO_NAME, m_ComboName);
}
...
BEGIN_MESSAGE_MAP(CListBoxDemoDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_ADD_STRING, &CListBoxDemoDlg::OnBnClickedButtonAddString)
	ON_BN_CLICKED(IDC_BUTTON_INSERT_STRING, &CListBoxDemoDlg::OnBnClickedButtonInsertString)
	ON_BN_CLICKED(IDC_BUTTON_FIND_STRING, &CListBoxDemoDlg::OnBnClickedButtonFindString)
...
void CListBoxDemoDlg::OnBnClickedButtonAddString()
{
	...
	//m_ctrlNameBox.SetWindowText(m_strName);
...
void CListBoxDemoDlg::OnLbnSelchangeListName()
{
	...
	AfxMessageBox(strTemp + _T("에게 전화걸기"));
}
...
void CListBoxDemoDlg::OnBnClickedButtonCombo()
{
	...
	// 2. 그 글자를 콤보박스에 추가한다.
	m_ctrlNameBox.AddString(m_ComboName);
	m_ctrlNameCombo.AddString(m_ComboName);
